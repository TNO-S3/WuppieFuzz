    ...
    S: HasRandAndOpenAPI,
{
    fn mutate(&mut self, state: &mut S, input: &mut OpenApiInput) -> Result<MutationResult, Error> {
        ...
        let mut available_methods: Vec<(Method, Option<usize>)> =
            match self.method_mutation_strategy {
        ...
        available_methods.retain(|&(method, _)| method != random_input.method);
        if available_methods.is_empty() {
            return Ok(MutationResult::Skipped);
        }

        let (new_method, http_method_idx) = rand.choose(available_methods).unwrap();
        random_input.method = new_method;
